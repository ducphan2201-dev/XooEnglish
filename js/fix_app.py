...
filepath = r"g:\My Drive\CODEAPP\XooEnglish\js\app.js"
with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Thay thế loadData và thêm prepareGlobalData
oldStart = content.find('async function loadData')
oldEnd = content.find('function renderClasses')
if oldStart != -1 and oldEnd != -1:
    newLoadData = """function prepareGlobalData() {
...
"""
    content = content[:oldStart] + newLoadData + content[oldEnd:]

# 2. Thay thế startSession
startS = content.find('async function startSession')
endS = content.find('// ----------------------------------------------------', startS)
if startS != -1 and endS != -1:
    newStartSession = """async function startSession(className, isDemo) {
...
"""
    content = content[:startS] + newStartSession + content[endS:]

# 3. Thay thế submitRenewForm
rnwS = content.find('async function submitRenewForm')
rnwE = content.find('async function submitForm')
if rnwS != -1 and rnwE != -1:
    newRenew = """async function submitRenewForm(e) {
    e.preventDefault();
...
"""
    content = content[:rnwS] + newRenew + content[rnwE:]

# 4. Thay thế submitForm
frmS = content.find('async function submitForm')
frmE = content.find('// ==== LOGIC LỊCH SỬ ĐIỂM DANH ====')
if frmS != -1 and frmE != -1:
    newForm = """async function submitForm(e) {
    e.preventDefault();
...
"""
    content = content[:frmS] + newForm + content[frmE:]

# 5. Thay thế openHistoryModal đến hết applyFastUpdate
hisS = content.find('async function openHistoryModal')
hisE = content.find('// ==== LOGIC QUẢN LÝ TÀI CHÍNH ====')
if hisS != -1 and hisE != -1:
    newHist = """function openHistoryModal(className) {
...
"""
    content = content[:hisS] + newHist + content[hisE:]

# 6. Thay thế tải tài chính
finS = content.find('async function loadFinanceData')
finE = content.find('function formatVND')
if finS != -1 and finE != -1:
    newFinance = """function loadFinanceData(monthParam) {
...
"""
    content = content[:finS] + newFinance + content[finE:]

# 7. Thay thế submit tài chính
smbS = content.find('async function submitFinanceConfig')
if smbS != -1:
    newSubmit = """async function submitFinanceConfig(e) {
    e.preventDefault();
...
"""
    content = content[:smbS] + newSubmit

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(content)

print('Success Python Fix!')
